...
def read_event_data(DATADIR, game_id):
    ...
    """
    eventfile = (
        f"Sample_Game_{game_id}/Sample_Game_{game_id}_RawEventsData.csv"  # filename
    )
    full_path = DATADIR.joinpath(eventfile)
    events = pd.read_csv(full_path)  # read data
    return events
...
def tracking_data(DATADIR, game_id, teamname):
    ...
    """
    teamfile = "/Sample_Game_%d/Sample_Game_%d_RawTrackingData_%s_Team.csv" % (
        game_id,
        game_id,
        teamname,
    )
    ...
    tracking = pd.read_csv(
        "{}/{}".format(DATADIR, teamfile), names=columns, index_col="Frame", skiprows=3
    )
    return tracking
